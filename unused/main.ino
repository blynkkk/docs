...
lv_obj_t *lbl_state, *lbl_rssi, *lbl_ssid, *lbl_ip, *lbl_mac, *lbl_hum, *lbl_temp, *btn_reset;
...
void display_init()
{
    ...
    backLight.setBrightness(backLight.getMaxBrightness() / 10);
    ...
    lv_menu_separator_create(sub_status_page);
    section = lv_menu_section_create(sub_status_page);
    cont = create_button(section, NULL, " ", "Reset WiFi");
    ...
    cont = create_slider(section, LV_SYMBOL_EYE_OPEN, "Brightness", 1, backLight.getMaxBrightness(), backLight.getBrightness());
    ...
    cont = create_status(section, NULL, "Humidity", STR_NONE);
    lbl_hum = lv_obj_get_child(cont, 1);

    cont = create_status(section, NULL, "Temperature", STR_NONE);
    lbl_temp = lv_obj_get_child(cont, 1);
...
void setup()
{
    ...
    lv_label_set_text_fmt(lbl_hum, "%d %%", hum);
    ...
    //lv_label_set_text_fmt(lbl_hum, "%d", jx);
    //lv_label_set_text_fmt(lbl_temp, "%d", jy);
...
BLYNK_WRITE(V2)
{
    jx = param[0].asInt(); // getting first value
}

BLYNK_WRITE(V3)
{
    jy = param[1].asInt(); // getting second value
}
