...
When you create a new Static Token using the tools we offer, two related items are created: **AuthToken** and **QR Token**. Also, Static Token always belongs to a specific Device Template and specific Organization.

#### \*\*\*\*

### AuthToken

Every device on Blynk platform has an AuthToken \(OAuth Token\). This is a unique identifer of the device and it's used to authenticate, validate, and connect devices to Blynk.Cloud. 
...
This is a special token encoded into a QR code image. This QR code is used only for assigning a certain device to a certain user within a certain organization. This process is also called _Device claiming_.
...
5. Managing Static Tokens post deployment
...
Every QR code image file is named afte an AuthToken it belongs to. 
...
## 5. Managing Static Tokens After Deployment
